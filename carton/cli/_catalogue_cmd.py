"""``carton-maya catalogue <command>`` — catalogue maintenance.

Houses what was previously top-level ``carton list`` and
``carton unpublish``. They moved under ``catalogue`` so the CLI surface
stays organised by area rather than mixing per-package and per-catalogue
...
def register_catalogue_subparser(sub):
    ...
    unpub = cat_sub.add_parser(
        "unpublish",
        help="Force-unpublish a package from a catalogue",
    )
    unpub.add_argument(
        "--catalogue", required=True, help="Path to catalogue.json",
    )
    unpub.add_argument(
        "--id", required=True,
...
def dispatch_catalogue(args, parser):
    """Route a parsed ``catalogue <subcmd>`` invocation to its handler."""
...
def _load_catalogue(path, migrate=True):
    ...
    the ``id`` command, which needs to reject pre-v5.0 files rather
...
def _catalogue_id(args):
    ...
    if catalogue.get("schema_version") != CATALOGUE_SCHEMA_VERSION:
        print(
            ...
            "'carton-maya catalogue migrate {}' first.".format(
...
def _catalogue_schema(args):
    """Print the catalogue.json JSON Schema to stdout."""
    from carton.core.lint import bundled_schema_path

    path = bundled_schema_path("catalogue.schema.json")
    if not path:
        print(
            "ERROR: bundled catalogue.schema.json not found in carton-maya install",
            file=sys.stderr,
        )
        sys.exit(1)
    with open(path, "r", encoding="utf-8") as f:
