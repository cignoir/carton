...
- Maya 2022 以降（Python 3 モードで起動していること。
  Maya 2022 以降はこれが既定です）

  Carton が必要とするのは Python 3 と PySide2 / PySide6 だけで、
  Maya のバージョン自体には依存しません。そのためインストーラは
  バージョンの許可リストではなくインタプリタを見ます。CI では
  Python 3.9（Maya 2023）以上でテストを回しています。Maya 2022 の
  Python 3.7 は対応していますが CI では踏んでいません。
...
ダイアログが 2 段階で質問します。**Step 1 — 何を追加する？**

- 📦 **単品パッケージ** — ツールを 1 つ直接追加（よくあるケース）。
- 📚 **カタログ** — 複数ツールをまとめた配布先に繋ぐ（配布者・チーム向け）。

**Step 2 — どこから？**（選択肢は Step 1 で分岐、`← 戻る` で Step 1 に戻れます）
...
v0.5.0 は **Package-first モデル**を導入し、カタログスキーマを 4.0 → 5.0 に bump します — パッケージはカタログから独立して存在し、origin が第一級概念となり、単品 GitHub リポジトリは `catalogue.json` ラッパー無しで動きます。

> **⚠️ 破壊的変更** — 共有カタログをアップグレードする前に必ず読んでください。
...
- `Add` ピッカーは **2 段階ダイアログ** に刷新。まず単品パッケージ / カタログのスコープを選び、次に transport を選ぶ形に。既存の 5 つの追加経路はすべて残っていますが、スコープごとにグループ化されています。
...
Carton が書き込むファイル（`config.json` / `installed.json` / カタログ / プロファイル）は、すべて一時ファイルに書いてから一手で差し替えます。そのため、書き込みが中断されても中途半端なファイルが残ることはありません。

それでも読めないファイルが出てきた場合（ディスク障害や外部エディタでの編集など）、Carton はそれを `<名前>.corrupt-<タイムスタンプ>` にリネームし、起動を拒否せずデフォルト値で立ち上がります。ファイルは削除されません。カタログ一覧はアクティブなプロファイルが独自にコピーを持っているため、通常は自動的に復元されます。
...
参照方式であるため、元ファイルを移動・リネームするとリンクが切れます。Carton はこれを検知し、カードに **「ソース消失」** バッジを表示して Launch の代わりに **「再リンク」** を提示します。再リンクすると登録が新しい場所を指すようになり、そのセッションの Maya パスも張り替えられます。パッケージの identity は変わらないため、既に publish 済みのものには影響しません。
...
**Namespace** フィールドは**必須**です。namespace の無いパッケージは publish できないため、後から気づくのではなく登録時に入力してもらう形にしています。`MyStudio` のように入力すると自動的に `mystudio` に正規化され、入力欄の下に正規化後の形式がライブで表示されます。

ほとんどの人は自分のツールを同じ namespace に登録するため、**Settings → デフォルト namespace** を一度設定しておけば、以降は自動で入力された状態で開きます。`package.json` や sidecar が自前の namespace を持っている場合はそちらが優先されます。
